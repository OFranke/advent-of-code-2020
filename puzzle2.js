...
let validPasswords = [];
let validPasswords2 = [];
input.map((elem, index, arr) => {
  const numberRegex = /\d+/g;
  const numbers = elem.match(numberRegex);
  const minNumber = numbers[0];
  const maxNumber = numbers[1];

  const charRegex = /[a-z]:/g;
  const chars = elem.match(charRegex);

  const passwordRegex = /[a-z]{2,}/g;
  const password = elem.match(passwordRegex)[0];

  const char = chars[0].replace(':', '');

  const occurenceRegex = new RegExp(`${char}`, 'g');
  const occurences = password.match(occurenceRegex);

  if (
    occurences &&
    occurences.length >= minNumber &&
    occurences.length <= maxNumber
  ) {
    validPasswords.push(password);
  }
  console.log('\x1b[33m%s\x1b[0m', '%c >> minNumber', minNumber);
  console.log('\x1b[33m%s\x1b[0m', '%c >> maxNumber', maxNumber);
  console.log('\x1b[33m%s\x1b[0m', '%c >> password', password);
  console.log(
    '\x1b[33m%s\x1b[0m',
    '%c >> password[minNumber - 1]',
    password[minNumber - 1]
  );
  console.log(
    '\x1b[33m%s\x1b[0m',
    '%c >> password[maxNumber - 1]',
    password[maxNumber - 1]
  );
  if (
    (password[minNumber - 1] == char && password[maxNumber - 1] != char) ||
    (password[minNumber - 1] != char && password[maxNumber - 1] == char)
  ) {
    // console.log('\x1b[33m%s\x1b[0m', '%c >> password', password);
...
console.log(
  '\x1b[33m%s\x1b[0m',
  '%c >> validPasswords.length',
  validPasswords.length
);
console.log(
  '\x1b[33m%s\x1b[0m',
  '%c >> validPasswords2.length',
